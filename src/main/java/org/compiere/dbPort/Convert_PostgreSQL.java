...
public class Convert_PostgreSQL extends Convert_SQL92 {
  ...
  protected ArrayList<String> convertStatement(String sqlStatement) {
    ...
    if ("true".equals(System.getProperty("org.idempiere.db.postgresql.debug"))) {
      log.warn("Oracle -> " + sqlStatement);
      log.warn("PgSQL  -> " + statement);
    }
    return result;
  } // convertStatement
  ...
  */
  ...
  private String convertUpdate(String sqlStatement) {
    ...
    while (charIndex < sqlLength) {
      ...
      if (Character.isWhitespace(c)) {
        if (token.length() > 0) {
          cnt++;
          if (cnt == 1) isUpdate = "UPDATE".equalsIgnoreCase(token.toString());
          else if (cnt == 2) targetTable = token.toString();
          else if (cnt == 3) {
    ...
    if (isUpdate && targetTable != null && sqlUpper.charAt(charIndex) == '(') {
        ...
        if (Character.isWhitespace(c)) {
          if (token.length() > 0) {
            ...
            if ("(".equals(currentToken)
                || (currentToken != null && currentToken.startsWith("("))) {
            ...
            if (")=(".equals(currentToken)) {
              ...
              if (")".equals(previousToken)
                  || (previousToken != null && previousToken.endsWith(")"))) {
                select = sqlStatement.substring(charIndex - currentToken.length());
                updateFields = sqlStatement.substring(updateFieldsBegin, charIndex);
                updateFields = updateFields.substring(0, updateFields.lastIndexOf(')'));
                break;
      ...
      if (!targetAlias.equals(targetTable)) Update.append(" " + targetAlias);
      ...
      while (f > 0) {
        ...
        if (f < 0) {
          ...
          if (useSubQuery) {
            ...
            if (mainWhere != null) mainWhere = " AND " + mainWhere;
            else mainWhere = "";
  ...
  private boolean isSQLFunctions(String token) {
    if (token.equalsIgnoreCase("current_timestamp")) return true;
  ...
  private String convertAlias(String sqlStatement) {
    ...
    if ("UPDATE".equalsIgnoreCase(tokens[0])) {
      ...
      if ("VALUES".equalsIgnoreCase(tokens[3]) || "SELECT".equalsIgnoreCase(tokens[3]))
        return sqlStatement;
      if (tokens[2].indexOf('(') > 0) return sqlStatement;
  ...
  private String convertDDL(String sqlStatement) {
    if (sqlStatement.toUpperCase().indexOf("ALTER TABLE ") == 0) {
      ...
      if (begin_col < 0) return sqlStatement;
        ...
        if (action.equals(" ADD ")) {
          if (rest.toUpperCase().indexOf(" DEFAULT ") != -1) {
            ...
            if (defaultvalue.equalsIgnoreCase("NULL")
                || defaultvalue.equalsIgnoreCase("statement_timestamp()")) {
              DDL =
                  sqlStatement.substring(0, begin_col - action.length())
                      + " ADD COLUMN "
                      + column
                      + " "
                      + beforeDefault.trim()
          ...
          if (rest.toUpperCase().startsWith("NOT ")
              ...
              || rest.toUpperCase().equals("NULL")
              || rest.toUpperCase().equals("NOT NULL")) {
            type = null;
          } else {
          ...
          if (rest.toUpperCase().indexOf(" DEFAULT ") != -1) {
            ...
            if (rest != null && rest.toUpperCase().indexOf("NOT NULL") >= 0)
              nullclause = "NOT NULL";
            else if (rest != null && rest.toUpperCase().indexOf("NULL") >= 0) nullclause = "NULL";
          ...
          if (type != null) DDL = DDL + "'" + type + "',";
          else DDL = DDL + "null,";
          if (nullclause != null) DDL = DDL + "'" + nullclause + "',";
          else DDL = DDL + "null,";
          if (defaultvalue != null) DDL = DDL + "'" + defaultvalue + "'";
          else DDL = DDL + "null";
          DDL = DDL + ")";
        }
        return DDL;
